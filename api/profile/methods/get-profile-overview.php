...
trait GetProfileOverview
{
	...
	public function getProfileOverview()
	{
		...
		if ($this->onBoard === false) {
			...
			$this->report[] = array(
				...
				'profileFields' => $this->profileFields // sends an array containing all profile fields as key:value pairs
			...
			$this->report[] = array(
				'api' => 'Profile',
				'response' => 'got-profile-overview',
				//'result' => true,
				//'message' => '<s><b class="icon-done-all"></b>Dashboard loaded successfully</s>',
				'got-profile-overview' => true, // tells jquery to load full profile
				'profileFields' => $this->profileFields // sends an array containing all profile fields as key:value pairs
			);
			return;
		}
	} // end method getProfileOverview();
	...
	public function getProfileFields() {
		...
		if (!$stmt) { // failed to prepare statement
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > prepare-statement',
		...
		if (!$stmt) { // failed to bind parameters
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > bind-parameters',
		...
		if (!$stmt) { // failed to execute query
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > execute-query',
		...
		if (!$result) { // failed to get result
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > get-query-results',
		...
		if (!$result->num_rows > 0) { // failed to find row in user table
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > find-row-in-user-table',
		...
		if (!$row) { // failed to fetch row
			...
			$this->report[] = array(
				'api' => 'Profile',
				'action' => 'get-profile > get-profile-fields-kyc > fetch-row-from-results',
		...
		try {
			...
			$this->report[] = array(
				'api' => 'Profile',
				'response' => 'get-profile > get-profile-fields-kyc > fetch-fields',
				'result' => false,
